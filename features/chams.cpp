...
bool Chams::GenerateLerpedMatrix( int index, BoneArray* out ) {
	...
	for ( auto it = data->m_records.rbegin( ); it != data->m_records.rend( ); it++ ) {
		...
		if ( record && record->valid( ) && ( !end && ( ( it + 1 )->get( ) ) ) ) {
			if ( record->m_origin.dist_to( ent->GetAbsOrigin( ) ) < 1.f ) {
				return false;
			}
			...
			float mul = 1.f / add;
			vec3_t lerp = math::Interpolate( next, record->m_origin, std::clamp( delta * mul, 0.f, 1.f ) );
			...
			for ( size_t i{ }; i < 128; ++i ) {
				vec3_t matrix_delta = record->m_bones[ i ].GetOrigin( ) - record->m_origin;
