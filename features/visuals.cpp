...
void Visuals::ThirdpersonThink( ) {
	...
	if ( m_thirdperson ) {
		...
		if ( alive && !g_csgo.m_input->CAM_IsThirdPerson( ) )
		...
		else if ( g_cl.m_local && g_cl.m_local->m_iObserverMode( ) == 4 ) {
			...
			if ( g_csgo.m_input->CAM_IsThirdPerson( ) ) {
				...
				g_csgo.m_input->vecCameraOffset.z = 0.f;
			}
	...
	if ( g_csgo.m_input->CAM_IsThirdPerson( ) ) {
		...
		if ( g_hvh.m_fake_duck )
			origin = g_cl.m_local->GetAbsOrigin( ) + vec3_t( 0.f, 0.f, 64.f );
		else
			origin = g_cl.m_local->GetAbsOrigin( ) + g_cl.m_local->m_vecViewOffset( );
		...
		g_csgo.m_input->vecCameraOffset = { offset.x, offset.y, offset.z };
	}
}
...
void Visuals::NoSmoke( ) {
	...
	static auto smoke_count = pattern::find( g_csgo.m_client_dll, XOR( "A3 ? ? ? ? 57 8B CB" ) ).add( 1 ).get< int* >( 2 );

	if ( g_menu.main.visuals.nosmoke.get( ) ) {
		if ( smoke_count )
			*smoke_count = 0;

		if ( smoke1 && !smoke1->GetFlag( MATERIAL_VAR_NO_DRAW ) )
			smoke1->SetFlag( MATERIAL_VAR_NO_DRAW, true );

		if ( smoke2 && !smoke2->GetFlag( MATERIAL_VAR_NO_DRAW ) )
			smoke2->SetFlag( MATERIAL_VAR_NO_DRAW, true );

		if ( smoke3 && !smoke3->GetFlag( MATERIAL_VAR_NO_DRAW ) )
			smoke3->SetFlag( MATERIAL_VAR_NO_DRAW, true );

		if ( smoke4 && !smoke4->GetFlag( MATERIAL_VAR_NO_DRAW ) )
...
void Visuals::think( ) {
	...
	if ( g_menu.main.visuals.noscope.get( )
		...
		&& g_cl.m_local->GetActiveWeapon( )->GetWpnData( )->nWeaponType == CSWeaponType::WEAPONTYPE_SNIPER_RIFLE
...
void Visuals::StatusIndicators( ) {
	...
	if ( g_menu.main.visuals.indicators.get( 0 ) ) {
		Indicator_t ind{ };
		float diff = 0.f;
		if ( g_menu.main.antiaim.desync.get( ) && g_cl.m_local && g_cl.m_processing ) {
			CCSGOPlayerAnimState* state = g_cl.m_local->m_PlayerAnimState( );
			float max_desync = state ? state->GetMaxDesyncDelta( ) : 58.f;
			diff = std::min( g_hvh.m_delta, max_desync );
		}
		float factor = std::clamp( diff / 58.f, 0.f, 1.f );
		int r = ( int )( ( 1.f - factor ) * 255.f + factor * 123.f );
		int g = ( int )( factor * 194.f );
		int b = ( int )( factor * 21.f );
		ind.color = Color( r, g, b, 255 );
		ind.text = XOR( "FAKE" );
		indicators.push_back( ind );
	}
	...
	if ( g_menu.main.aimbot.minimal_damage_override_key.get( ) != -1 && g_aimbot.m_dmg_override ) {
		Indicator_t ind{ };
	...
	if ( g_menu.main.visuals.indicators.get( 3 ) && g_menu.main.aimbot.doubletap_key.get_status( ) ) {
		Indicator_t ind{ };
		float factor = std::clamp( ( float )g_cl.m_tickbase_shift / 13.f, 0.f, 1.f );
		int r = ( int )( ( 1.f - factor ) * 255.f + factor * 123.f );
		int g = ( int )( factor * 194.f );
		int b = ( int )( factor * 21.f );
		ind.color = Color( r, g, b, 255 );
	...
	if ( g_menu.main.visuals.indicators.get( 4 ) && g_menu.main.aimbot.hideshots_key.get_status( ) ) {
		Indicator_t ind{ };
		float factor = std::clamp( ( float )g_cl.m_tickbase_shift / 9.f, 0.f, 1.f );
		int r = ( int )( ( 1.f - factor ) * 255.f + factor * 123.f );
		int g = ( int )( factor * 194.f );
		int b = ( int )( factor * 21.f );
		ind.color = Color( r, g, b, 255 );
	...
	if ( g_menu.main.visuals.indicators.get( 5 ) && g_hvh.m_fake_duck ) {
		Indicator_t ind{ };
	...
	if ( g_menu.main.visuals.indicators.get( 6 ) && g_menu.main.antiaim.freestand.get_status( ) ) {
		Indicator_t ind{ };
		ind.color = Color( 255, 255, 255, 255 );
		ind.text = XOR( "FS" );
		indicators.push_back( ind );
...
void Visuals::draw( Entity* ent ) {
	if ( ent->IsPlayer( ) ) {
		...
		if ( ent->is( HASH( "CMolotovProjectile" ) )
			|| ent->is( HASH( "CInferno" ) )
			|| ent->is( HASH( "CBaseCSGrenadeProjectile" ) )
			|| ent->is( HASH( "CSmokeGrenadeProjectile" ) )
			|| ent->is( HASH( "CDecoyProjectile" ) ) ) {
...
void Visuals::DrawProjectile( Weapon* ent ) {
	...
	bool is_molly = ent->is( HASH( "CMolotovProjectile" ) ) || ent->is( HASH( "CInferno" ) );
	bool is_grenade = ent->is( HASH( "CBaseCSGrenadeProjectile" ) );
	bool is_smoke = ent->is( HASH( "CSmokeGrenadeProjectile" ) );
	bool is_decoy = ent->is( HASH( "CDecoyProjectile" ) );

	if ( !is_molly && !is_grenade && !is_smoke && !is_decoy )
		return;

	vec3_t destination = ent->GetAbsOrigin( );
	vec3_t origin = g_cl.m_local->GetAbsOrigin( );

	vec3_t sub = destination - origin;
	int distance = static_cast< int >( std::round( sub.length( ) / 12.f ) );

	if ( distance > 150 )
		return;

	vec2_t screen;
	if ( !render::WorldToScreen( destination, screen ) )
		return;

	int cx = static_cast< int >( screen.x );
	int cy = static_cast< int >( screen.y ) - 20;

	float red_calc = 255.f - std::pow( static_cast< float >( distance ), 1.7f );
	int red_val = std::clamp( static_cast< int >( red_calc ), 0, 255 );

	render::circle( cx, cy, 30, 64, Color( red_val, 0, 0, 255 ) );
	render::circle_outline( cx - 1, cy, 30, 64, Color( 10, 10, 10, 255 ) );
	render::circle_outline( cx - 1, cy, 31, 64, Color( 10, 10, 10, 255 ) );

	render::warning_icon.string( cx, cy - 18, Color( 255, 250, 20, 200 ), XOR( "!" ), render::ALIGN_CENTER );
	render::esp.string( cx, cy + 5, Color( 232, 232, 232, 200 ), tfm::format( XOR( "%i ft" ), distance ), render::ALIGN_CENTER );

	if ( is_molly ) {
		static Address offset = g_netvars.get( HASH( "DT_Inferno" ), HASH( "m_nFireEffectTickBegin" ) );
		int fire_tick = offset ? ent->get< int >( offset ) : 0;
		float time = fire_tick ? game::TICKS_TO_TIME( fire_tick ) : ent->get< float >( g_entoffsets.m_flSimulationTime );
		float factor = std::clamp( ( ( time + 7.f ) - g_csgo.m_globals->m_curtime ) / 7.f, 0.f, 1.f );
		if ( factor > 0.f ) {
			float start_rad = math::deg_to_rad( 270.f );
			float end_rad = start_rad + math::deg_to_rad( 360.f * factor );
			render::circle_arc( cx - 1, cy, 30, start_rad, end_rad, 64, Color( 232, 232, 232, 200 ) );
		}
	}
}
...
void Visuals::DrawItem( Weapon* item ) {
	...
	if ( data->nWeaponType == WEAPONTYPE_GRENADE || data->nWeaponType == WEAPONTYPE_KNIFE )
		return;

	if ( item->m_iItemDefinitionIndex( ) == 0 || item->m_iItemDefinitionIndex( ) == WEAPON_C4 )
		return;
		...
		if ( ( enemy && g_menu.main.players.weapon.get( 0 ) ) || ( !enemy && g_menu.main.players.weapon.get( 1 ) ) ) {
			...
			if ( weapon ) {
				...
				if ( data ) {
					...
					// ammo bar.
					if ( max != -1 && g_menu.main.players.ammo.get( ) && data->IsGun( ) ) {
						// check for reload.
						if ( reload )
...
void Visuals::RenderGlow( ) {
	...
	if ( !g_menu.main.players.glow.get( 0 ) && !g_menu.main.players.glow.get( 1 ) )
		return;
	...
	for ( int i{ }; i < g_csgo.m_glow->m_object_definitions.Count( ); ++i ) {
		...
		obj->arrColor[ 0 ] = ( float )color.r( ) / 255.f;
		obj->arrColor[ 1 ] = ( float )color.g( ) / 255.f;
		obj->arrColor[ 2 ] = ( float )color.b( ) / 255.f;
		obj->arrColor[ 3 ] = opacity * blend;
	}
}
	...
	if ( !g_cl.m_processing ) {
		...
		for ( size_t i{ impact_count }; i-- > 0; ) {
			...
			if ( is_final_impact ) {
				...
				beam_info.vecStart = start;
				beam_info.vecEnd = end;
				beam_info.nModelIndex = g_csgo.m_model_info->GetModelIndex( XOR( "sprites/purplelaser1.vmt" ) );
				beam_info.pszModelName = XOR( "sprites/purplelaser1.vmt" );
				beam_info.flHaloScale = 0.f;
				beam_info.flLife = g_menu.main.visuals.impact_beams_time.get( );
				beam_info.flWidth = 2.f;
				beam_info.flEndWidth = 2.f;
				beam_info.flFadeLength = 0.f;
				beam_info.flAmplitude = 0.f;   // beam 'jitter'.
				beam_info.flBrightness = 255.f;
				beam_info.flSpeed = 0.5f;  // seems to control how fast the 'scrolling' of beam is... once fully spawned.
				beam_info.iStartFrame = 0;
				beam_info.flFrameRate = 0.f;
				beam_info.nSegments = 2;     // controls how much of the beam is 'split up', usually makes m_flAmplitude and m_flSpeed much more noticeable.
				beam_info.bRenderable = true;  // must be true or you won't see the beam.
				beam_info.nFlags = 0;

				if ( !impact->m_hit_player ) {
					beam_info.flRed = g_menu.main.visuals.impact_beams_color.get( ).r( );
					beam_info.flGreen = g_menu.main.visuals.impact_beams_color.get( ).g( );
					beam_info.flBlue = g_menu.main.visuals.impact_beams_color.get( ).b( );
				}

				else {
					beam_info.flRed = g_menu.main.visuals.impact_beams_hurt_color.get( ).r( );
					beam_info.flGreen = g_menu.main.visuals.impact_beams_hurt_color.get( ).g( );
					beam_info.flBlue = g_menu.main.visuals.impact_beams_hurt_color.get( ).b( );
				}
...
void Visuals::DrawAutoPeek( ) {
	if ( !g_cl.m_processing || !g_movement.m_peeking || !g_csgo.m_effects || !g_cl.m_local )
		return;

	static float alpha = 0.0f;
	if ( g_movement.m_peeking )
		alpha += 3.f * g_csgo.m_globals->m_frametime;
	else
		alpha -= 3.f * g_csgo.m_globals->m_frametime;

	alpha = std::clamp( alpha, 0.0f, 1.0f );
	float rad = std::max( 2.f, 33.f * alpha ) - 1.f;

	auto light = g_csgo.m_effects->CL_AllocDlight( g_cl.m_local->index( ) );
	if ( light ) {
		light->die = g_csgo.m_globals->m_curtime + 0.05f;
		light->radius = rad;
		if ( g_movement.m_retracting ) {
			light->color.r = 0;
			light->color.g = 255;
			light->color.b = 0;
		}
		else {
			Color custom_col = g_menu.main.movement.autopeek_color.get( );
			light->color.r = custom_col.r( );
			light->color.g = custom_col.g( );
			light->color.b = custom_col.b( );
		}
		light->color.exponent = 8;
		light->key = g_cl.m_local->index( );
		light->origin = g_movement.m_peek_pos;
		light->flags &= 0x01 | 0x02;
		light->m_OuterAngle = 360.f;
		light->style = 2;
	}
}