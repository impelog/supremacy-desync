    ...
    while( damage > 0.f ) {
		...
		if( in->m_target ) {
			...
			if( trace.m_entity && trace.m_entity == in->m_target && game::IsValidHitgroup( trace.m_hitgroup ) ) {
				int group = ( weapon->m_iItemDefinitionIndex( ) == WEAPON_TASER ) ? HITGROUP_GENERIC : trace.m_hitgroup;
		...
		if( !TraceToExit( trace.m_endpos, dir, pen_end, &trace, &exit_trace ) ) {
			if( !( g_csgo.m_engine_trace->GetPointContents( trace.m_endpos, MASK_SHOT_HULL ) & MASK_SHOT_HULL ) )
				return false;
		}
