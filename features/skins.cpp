...
void Skins::think( ) {
			...
			if ( weapon->IsKnife( ) ) {
				...
				if ( g_menu.main.skins.knife.get( ) > 0 ) {
					...
					if ( g_cl.m_local->m_iTeamNum( ) == TEAM_TERRORISTS )
						weapon->m_iItemDefinitionIndex( ) = WEAPON_KNIFE_T;

					else if ( g_cl.m_local->m_iTeamNum( ) == TEAM_COUNTERTERRORISTS )
						weapon->m_iItemDefinitionIndex( ) = WEAPON_KNIFE;
	...
	if ( g_menu.main.skins.glove.get( ) > 0 ) {
		...
		if ( !glove ) {
			...
			if ( old && g_csgo.m_entlist->GetClientEntityFromHandle( old->m_hOwnerEntity( ) ) == g_cl.m_local ) {
		...
		glove->m_hOwnerEntity( ) = g_cl.m_local;
		...
		glove->m_nModelIndex( ) = data->m_model_index;
...
void Skins::ForceUpdateHud( ) {
	auto weapon_selection = g_csgo.m_hud->FindElement( FNV1a::get( "CCSGO_HudWeaponSelection" ) );
	if ( !weapon_selection )
		return;

	int* hud_weapons = reinterpret_cast< int* >( uintptr_t( weapon_selection ) - 0x28 );

	if ( !hud_weapons )
		return;

	int count = *( hud_weapons + 0x20 );

	for ( int i = 0; i < count; i++ ) {
		g_csgo.ClearHudWeapons
			.as<int( __thiscall* )( int*, int )>( )( hud_weapons, i );
	}
}
