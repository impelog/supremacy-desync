...
public:
	...
	Slider        baim_hp;
	Keybind       baim_key;
	Keybind       minimal_damage_override_key;
	Slider        minimal_damage_override_amt;
	Keybind       doubletap_key;
	MultiDropdown doubletap_options;
	Keybind       hideshots_key;
	Keybind       force_teleport;
...
public:
	void init( ) {
		...
		minimal_damage_override_key.setup( XOR( "minimal damage override key" ), XOR( "minimal_damage_override_key" ) );
		minimal_damage_override_key.SetToggleCallback( callbacks::ToggleDmgOverride );
		RegisterElement( &minimal_damage_override_key, 1 );

		minimal_damage_override_amt.setup( "", XOR( "minimal_damage_override_amt" ), 1.f, 100.f, false, 0, 40.f, 1.f );
		RegisterElement( &minimal_damage_override_amt, 1 );

		doubletap_key.setup( XOR( "double tap" ), XOR( "doubletap_key" ) );
		RegisterElement( &doubletap_key, 1 );

		doubletap_options.setup( XOR( "double tap options" ), XOR( "doubletap_options" ), { XOR( "Break LC" ), XOR( "Lag Peek" ), XOR( "Immediate teleport" ), XOR( "Fast throw" ) } );
		RegisterElement( &doubletap_options, 1 );

		hideshots_key.setup( XOR( "hide shots" ), XOR( "hideshots_key" ) );
		RegisterElement( &hideshots_key, 1 );

		force_teleport.setup( XOR( "force teleport" ), XOR( "force_teleport" ) );
		RegisterElement( &force_teleport, 1 );
	}
};
...
public:
	void init( ) {
		...
		yaw.setup( XOR( "yaw" ), XOR( "yaw" ), { XOR( "off" ), XOR( "backwards" ), XOR( "jitter" ) } );
		RegisterElement( &yaw );

		yaw_offset.setup( XOR( "yaw offset" ), XOR( "yaw_offset" ), -180.f, 180.f, false, 0, 0.f, 5.f, XOR( L"\u00B0" ) );
		RegisterElement( &yaw_offset );

		jitter_range.setup( XOR( "jitter range" ), XOR( "jitter_range" ), 1.f, 180.f, false, 0, 45.f, 5.f, XOR( L"\u00B0" ) );
		RegisterElement( &jitter_range );

		base_angle.setup( XOR( "base angle" ), XOR( "base_angle" ), { XOR( "away crosshair" ), XOR( "away distance" ) } );
		RegisterElement( &base_angle );

		edge.setup( XOR( "edge" ), XOR( "edge" ) );
		RegisterElement( &edge );

		freestand.setup( XOR( "freestand" ), XOR( "freestand" ) );
		RegisterElement( &freestand );

		// col 2.
		desync.setup( XOR( "desync" ), XOR( "desync" ) );
		RegisterElement( &desync, 1 );

		desync_delta.setup( XOR( "desync amount" ), XOR( "desync_delta" ), -60.f, 60.f, false, 0, 58.f, 1.f, XOR( L"\u00B0" ) );
		desync_delta.AddShowCallback( callbacks::IsDesyncEnabled );
		RegisterElement( &desync_delta, 1 );

		fake_duck.setup( XOR( "fake duck" ), XOR( "fake_duck" ) );
		RegisterElement( &fake_duck, 1 );
...
public:
	void init( ) {
		...
		//aspect_ratio_amt.setup( "", XOR( "aspect_ratio_amt" ), 0.5f, 3.f, true, 2, 1.f, 0.1f );
		...
		thirdperson_distance.setup( "", XOR( "thirdperson_distance" ), 50.f, 150.f, false, 0, 100.f, 1.f );
		RegisterElement( &thirdperson_distance, 1 );

		indicators.setup( XOR( "indicators" ), XOR( "indicators" ), { XOR( "fake" ), XOR( "lag compensation" ), XOR( "fake latency" ), XOR( "double tap" ), XOR( "hide shots" ), XOR( "fake duck" ), XOR( "freestand" ) } );
...
public:
	void init( ) {
		...
		slowwalk.setup( XOR( "slow motion" ), XOR( "slowwalk" ) );
		RegisterElement( &slowwalk, 1 );

		autopeek.setup( XOR( "automatic peek" ), XOR( "autopeek" ) );
		RegisterElement( &autopeek, 1 );

		autopeek_color.setup( XOR( "automatic peek color" ), XOR( "autopeek_color" ), Color( 180, 0, 0, 255 ) );
