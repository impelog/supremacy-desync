...
public:
	...
	using ClampBonesInBBox_t           = void( __fastcall* )( void*, void*, matrix3x4_t*, int );
	using ShouldSkipAnimationFrame_t   = bool( __fastcall* )( void*, void* );
	...
	using CHLCreateMove_t			   = void( __thiscall* )( void*, int, float, bool );
	using AllocKeyValues_t			   = void* ( __thiscall* )( void*, size_t );
...
public:
	static void WINAPI CHLCreateMoveProxy( int sequence_number, float input_sample_time, bool active, bool& send_packet );
	static void __cdecl CL_Move( float accumulated_extra_samples, bool final_tick );
	static bool __fastcall WriteUsercmdDeltaToBuffer( void* thisptr, void* edx, int slot, void* buf, int from, int to, bool isnewcommand );

	bool					 IsPaused( );
	bool					 SendNetMsg( INetMessage* msg, bool force_reliable, bool voice );
	void*					 AllocKeyValues( size_t size );
	void					 CHLCreateMove( int sequence_number, float input_sample_time, bool active );
	...
	static void __fastcall   ClampBonesInBBox( void* ecx, void* edx, matrix3x4_t* bones, int boneMask );
...
public:
	...
	CSGO::CL_Move_t             m_CL_Move_original;
	WriteUsercmdDeltaToBuffer_t m_WriteUsercmdDeltaToBuffer_original;
	ClampBonesInBBox_t          m_ClampBonesInBBox_original;
	ShouldSkipAnimationFrame_t  m_ShouldSkipAnimationFrame_original;
...
public:
    ...
    __forceinline void init( ) {
        g_csgo.m_entlist->AddListenerEntity( this );
		...
		if ( g_csgo.m_engine->IsInGame( ) ) {
			for ( int i = 1; i <= g_csgo.m_entlist->GetHighestEntityIndex( ); i++ ) {
				if ( Entity* entity = g_csgo.m_entlist->GetClientEntity( i ); entity != nullptr )
