...
namespace pattern {
    ...
    static Address find( const PE::Module &module, const std::string &pat ) {
		Address result = find( module.GetBase( ), module.GetImageSize( ), pat );
		if ( Address::valid( result ) )
			return result;

		//MessageBoxA( nullptr, ( "Pattern not found: " + pat ).c_str( ), "Pattern Scan", MB_OK | MB_ICONERROR );
		return {};
	}
