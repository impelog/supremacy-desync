...
public:
	...
	using ClearNotices_t = void( __thiscall* )( std::uintptr_t );
	using DeathNotices_t = DWORD( __thiscall* )( void*, const char* );
	using SetCollisionBounds_t = void( __thiscall* )( void*, vec3_t&, vec3_t& );
...
public:
	...
	CL_Move_t               CL_Move;
	Address                 WriteUsercmdDeltaToBuffer;
	Address                 WriteUserCmd;
	Address                 ClampBonesInBBox;
	Address                 ShouldSkipAnimationFrame;
	...
	Address ClearKillfeed;
	Address ClearHudWeapons;
	Address PlayerVTable;
...
namespace game {
	__forceinline float GetClientInterpAmount( ) {
		...
		static ConVar* sv_min_interp_ratio = nullptr;
		static ConVar* sv_max_interp_ratio = nullptr;
		if ( !cl_interp ) {
			cl_interp = g_csgo.m_cvar->FindVar( HASH( "cl_interp" ) );
			cl_updaterate = g_csgo.m_cvar->FindVar( HASH( "cl_updaterate" ) );
			sv_minupdaterate = g_csgo.m_cvar->FindVar( HASH( "sv_minupdaterate" ) );
			sv_maxupdaterate = g_csgo.m_cvar->FindVar( HASH( "sv_maxupdaterate" ) );
			cl_interp_ratio = g_csgo.m_cvar->FindVar( HASH( "cl_interp_ratio" ) );
			sv_min_interp_ratio = g_csgo.m_cvar->FindVar( HASH( "sv_client_min_interp_ratio" ) );
			sv_max_interp_ratio = g_csgo.m_cvar->FindVar( HASH( "sv_client_max_interp_ratio" ) );
		}
		float update_rate = cl_updaterate && sv_minupdaterate && sv_maxupdaterate ? std::clamp( cl_updaterate->GetFloat( ), sv_minupdaterate->GetFloat( ), sv_maxupdaterate->GetFloat( ) ) : g_csgo.cl_updaterate->GetFloat( );
		float interp_ratio = cl_interp_ratio && sv_min_interp_ratio && sv_max_interp_ratio ? std::clamp( cl_interp_ratio->GetFloat( ), sv_min_interp_ratio->GetFloat( ), sv_max_interp_ratio->GetFloat( ) ) : g_csgo.cl_interp_ratio->GetFloat( );
		float lerp = std::clamp( interp_ratio / update_rate, cl_interp ? cl_interp->GetFloat( ) : g_csgo.cl_interp->GetFloat( ), 1.f );
		return std::roundf( lerp * 1000.f ) / 1000.f;
	}
	...
	inline void UpdateAnimationState( CCSGOPlayerAnimState* state, ang_t ang ) {
		static auto func = pattern::find( g_csgo.m_client_dll, XOR( "55 8B EC 83 E4 F8 83 EC 18 56 57 8B F9 F3 0F 11 54 24" ) );
		if ( !func || !state )
			return;
	...
	__forceinline void UTIL_ClipTraceToPlayers( const vec3_t& start, const vec3_t& end, uint32_t mask, ITraceFilter* filter, CGameTrace* tr, float range ) {
		static auto func = pattern::find( g_csgo.m_client_dll, XOR( "53 8B DC 83 EC 08 83 E4 F0 83 C4 04 55 8B 6B 04 89 6C 24 04 8B EC 81 EC ? ? ? ? 0F 57 C9" ) );
		if ( !func )
			return;
	...
	__forceinline ulong_t CalcUserCmdChecksum( CUserCmd* cmd ) {
		static auto func = pattern::find( g_csgo.m_client_dll, XOR( "53 8B D9 83 C8" ) ).as< ulong_t( __thiscall* )( CUserCmd* ) >( );
		if ( !func )
			return 0;
